...
public class ErrorHandler {
    ...
    public ResponseEntity<ErrorResponseDto> notFoundExceptionHandler(RuntimeException e) {
        ...
        ErrorResponseDto errorResponseDto = ErrorResponseDto.builder().status("NOT_FOUND")
                .reason("The required object was not found.").message(e.getMessage())
                .timestamp(DateTimeMapper.fromLocalDateTime(LocalDateTime.now())).build();
    ...
    public ResponseEntity<ErrorResponseDto> conflictExceptionHandler(Exception e) {
        log.error(e.getMessage());
        ErrorResponseDto errorResponseDto = ErrorResponseDto.builder().status("CONFLICT")
                .reason("Integrity constraint has been violated.").message(e.getMessage())
                .timestamp(DateTimeMapper.fromLocalDateTime(LocalDateTime.now())).build();
    ...
    public ResponseEntity<ErrorResponseDto> badRequestExceptionHandler(Exception e) {
        log.error(e.getMessage());
        ErrorResponseDto errorResponseDto = ErrorResponseDto.builder().status("BAD_REQUEST")
                .reason("Incorrectly made request.").message(e.getMessage())
                .timestamp(DateTimeMapper.fromLocalDateTime(LocalDateTime.now())).build();
